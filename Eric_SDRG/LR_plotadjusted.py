"""
LR2_distance_only.py

Distance-only GUI for validating Section-6 long-range SDRG (all-to-all only).

This script is adapted from your LR2.py ("long_range_section6_gui_alltoall_only_v2_savepdf.py") :contentReference[oaicite:0]{index=0}

Changes vs LR2.py:
- Removed the second (redundant) coupling-distribution subplot (J-space).
- Increased font sizes (axis labels / tick labels / legend / title) so the exported PDF
  remains readable in a two-column report.

Everything else is unchanged:
- Compute snapshots over "shells" in Omega.
- Show histogram (numerics) + master-equation theory curve + fixed-point curve.
- Slider + play/pause/stop.
- Save current plot to PDF (what you see is what gets saved).
"""
...
# =============================================================================
# 5) Fixed point curve in r-space
# =============================================================================
...
# =============================================================================
# 6) GUI (distance plot only)
# =============================================================================

class Section6LongRangeGUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        root.title("Section-6 Long-range SDRG validation (distance-only, all-to-all)")
        ...
        # Matplotlib figure (this is what we save to PDF)
        # Keep it reasonably wide for the GUI; PDF is vector anyway.
        self.fig = plt.Figure(figsize=(7.2, 4.2))
        self.ax_r = self.fig.add_subplot(1, 1, 1)

        self._build_layout()

    def _apply_axis_style(self):
        self.ax_r.tick_params(axis="both", which="major", labelsize=TICK_FONTSIZE)
        self.ax_r.tick_params(axis="both", which="minor", labelsize=TICK_FONTSIZE)
    ...
    def _build_layout(self):
        ...
        # Buttons row 1: compute + play controls
        btns = ttk.Frame(left)
        ...
        # Buttons row 2: Save PDF
        btns2 = ttk.Frame(left)
        btns2.pack(fill=tk.X, pady=(2, 2))
        btns2.columnconfigure(0, weight=1)
        self.btn_save_pdf = ttk.Button(btns2, text="Save plot (PDF)", command=self.on_save_pdf)
        self.btn_save_pdf.grid(row=0, column=0, padx=2, sticky="ew")
    ...
    def _plot_empty(self):
        self.ax_r.clear()
        self.ax_r.set_title("Distance distribution (density in log10 r)", fontsize=TITLE_FONTSIZE)
        self.ax_r.set_xlabel("r (log x-axis)", fontsize=LABEL_FONTSIZE)
        self.ax_r.set_ylabel(r"$dP/d\log_{10}(r)$", fontsize=LABEL_FONTSIZE)
    ...
    def on_save_pdf(self):
        ...
        - The file dialog lets you pick the name and location.
        - Vector PDF via Matplotlib.
        """
        default_name = "section6_longrange_distance.pdf"
        ...
        if not filename:
            return
        try:
            # bbox_inches tight + a bit of padding -> avoids cropped labels in LaTeX
    ...
    def on_compute(self):
        ...
        # Large-ring theory sampler (same logic as LR2.py)
        ...
        def pooled_pdf(samples_now: List[RingSDRG]) -> np.ndarray:
            gaps = np.concatenate([s.all_active_gaps() for s in samples_now])
            gaps = gaps[gaps > 0]
            if gaps.size == 0:
                return np.zeros_like(r_cent)
            hr, _ = np.histogram(gaps, bins=r_bins, density=True)
            return hr

        def theory_pdf(step: int) -> np.ndarray:
            if step == 0:
        ...
        def fixed_point_curve(rho: float) -> np.ndarray:
        ...
        hr0 = pooled_pdf(samples)
        tr0 = theory_pdf(step=0)

        rho0 = Omega0 ** (-1.0 / alpha)
        fp_r0 = fixed_point_curve(rho=rho0)
        ...
        for n in range(1, n_shells + 1):
            ...
            hr = pooled_pdf(samples)
            tr = theory_pdf(step=n)

            rho = (Omega_low if Omega_low > 0 else max(Omega_high, 1e-300)) ** (-1.0 / alpha)
            fp_r = fixed_point_curve(rho=rho)
    ...
    def _render_snapshot(self, idx: int):
        if (not self.snapshots) or (self.r_bins is None) or (self.r_cent is None):
            self._plot_empty()
            return
        ...
        # density in log10 r:  dP/d log10 r = (ln 10) * r * p(r)
        ...
        self._apply_axis_style()
        self.ax_r.legend(loc="best", fontsize=LEGEND_FONTSIZE, frameon=False)
    ...
    def _set_buttons_state(self, state: str):
        for b in [self.btn_compute, self.btn_play, self.btn_pause, self.btn_stop]:
            try:
                ...
                pass
        # Slider disabled during compute.
        if state == "disabled":
            self.slider.configure(state="disabled")
