"""
long_range_section6_gui_mastereq_shells_v4_progress.py

This is the "keep this version" code, with three improvements:

(1) Fixed-point curve sanity:
    - The fixed point is implemented in r-space as
          P*(r | rho) ∝ r^{-3/2}  1_{r >= rho}
      and mapped to J-space using J = r^{-alpha} with the correct Jacobian:
          P*(J) = P*(r(J)) * |dr/dJ|.
    - Since the plots use log-x axes, we *display* densities in log variables:
          u = log10 r:  dP/du = (ln 10) r P(r)
          v = log10 J:  dP/dv = (ln 10) J P(J)
      This avoids the misleading “blow-up” you saw when plotting a dP/dJ density
      on a log-J axis.

(2) Added a progress bar:
    - Shows shell-by-shell progress during batch compute.

(3) Clarifies connectivity modes:
    - "Nearest-neighbor": uses the NN effective-bond formula (simple second-order SDRG).
    - "All-to-all": uses the Section-6 long-range rule (Eq. 35/36-style f(RL,RR,rho)).
      This is the mode whose master-equation comparison is meaningful.

Run:
    python long_range_section6_gui_mastereq_shells_v4_progress.py
"""
...
# =============================================================================
# Safe parse helpers for GUI strings
# =============================================================================
...
def analytic_gap_pdf(dist_name: str, r: np.ndarray, scale: float, min_gap: float) -> np.ndarray:
    ...
    if dist_name == "Regular lattice (constant)":
        # approximate delta by a narrow Gaussian around r0=min_gap+scale
...
def normalize_density_on_bins(p: np.ndarray, bin_edges: np.ndarray) -> np.ndarray:
    ...
    Correct for non-uniform (e.g. log-spaced) bins.
    """
...
def renormalized_distance_alltoall(RL: float, rho: float, RR: float, alpha: float, gamma: float) -> float:
    ...
    anisotropy prefactor:
      for gamma=1 this becomes 1/2 as in Eq.(36).
    """
    RL = max(float(RL), 1e-300)
    RR = max(float(RR), 1e-300)
    rho = max(float(rho), 1e-300)
...
# =============================================================================
# Fast ring SDRG implementation (heap + linked list)
# =============================================================================
...
class RingSDRG:
    ...
    def step_one_decimation(self) -> bool:
        ...
        # SDRG monotonicity guard: do not create a new strongest bond.
        eps = 1e-12
...
def fixed_point_pdf_r(r: np.ndarray, rho: float) -> np.ndarray:
    """
    Fixed point used here (as in Section-6 long-range scaling):
        P*(r | rho) ∝ r^{-3/2} * 1_{r >= rho}.

    - rho plays the role of the RG-dependent microscopic cutoff (set by the current Ω):
          rho = Ω^{-1/alpha}.
    """
    r = np.asarray(r, dtype=float)
...
class Section6LongRangeGUI:
    def __init__(self, root: tk.Tk):
        ...
        self.var_mode = tk.StringVar(value="All-to-all")  # Section 6
    ...
    def _build_layout(self):
        ...
        # Progress bar (new)
        ttk.Label(left, text="Compute progress:").pack(anchor="w", pady=(6, 0))
    ...
    def on_compute(self):
        ...
        # Bins in r and J (densities w.r.t. r and J)
        ...
        def theory_pdf(step: int) -> Tuple[np.ndarray, np.ndarray]:
            # Step 0 = analytic initial condition (smooth).
            if step == 0:
        ...
        def fixed_point_curves(rho: float) -> Tuple[np.ndarray, np.ndarray]:
            # Fixed point in r-space, then map to J using Jacobian.
        ...
        # If we exited early, set progress to "done" visually
        ...
        self.lbl_info.config(
            text=f"Computed {len(snapshots)} shells in {dt:.2f}s. "
                 f"(N_theory={N_theory}, dOmega={dOmega}, mode={mode})"
        )
    ...
    def _render_snapshot(self, idx: int):
        ...
        # Convert displayed densities to densities w.r.t. log10-variable:
