...
class RingSDRG:
    ...
    def step_one_decimation(self) -> bool:
        ...
        # ------------------------------------------------------------------
        # Update rule: compute the effective distance \tilde r.
        #
        # IMPORTANT monotonicity/causality guard (same spirit as your earlier GUI):
        # SDRG should flow to *lower* energy scales. Since J = r^{-alpha}, we must
        # not create a new effective distance smaller than the strongest bond rho.
        # Otherwise, we would generate a coupling larger than the current \Omega,
        # which breaks the energy-shell logic and destroys distribution matching.
        # ------------------------------------------------------------------
        ...
        # Enforce \tilde r >= rho (tiny epsilon to avoid equality / heap ties).
        eps = 1e-12
        if not np.isfinite(new_gap):
            # Fallback: if the formula produced NaN/inf, revert to the bare merged length.
            new_gap = RL + rho + RR
...
class Section6LongRangeGUI:
    def __init__(self, root: tk.Tk):
        ...
        # Bin edges used for drawing histogram bars. These are set during on_compute().
        # We keep them as part of the GUI state so the rendering function does not have
        # to re-derive them or keep raw samples around.
    ...
    def _render_snapshot(self, idx: int, r_cent: np.ndarray, J_cent: np.ndarray):
        ...
        # r panel
        #
        # The user requested to "keep the histograms" (i.e., show binned bars)
        # rather than only plotting the histogram heights as a line.
        # We therefore draw the numerical distribution as semi-transparent bars
        # in (linear) r-space, while setting the x-axis to logarithmic scaling.
        # --------------------------------------------------------------
        if self.r_bins is not None:
            self.ax_r.bar(
                self.r_bins[:-1],
                snap["hist_r"],
                width=np.diff(self.r_bins),
                align="edge",
                color="C0",
                alpha=0.35,
                edgecolor="C0",
            ...
            # Fallback: line if bin edges are unavailable.
        ...
        # --------------------------------------------------------------
        # J panel
        # Same display choice as r: show the numerical histogram as bars.
        # --------------------------------------------------------------
        if self.J_bins is not None:
            self.ax_J.bar(
                self.J_bins[:-1],
                snap["hist_J"],
                width=np.diff(self.J_bins),
                align="edge",
                color="C0",
                alpha=0.35,
                edgecolor="C0",
        ...
        # If the numerical J-histogram is identically zero, surface a clear hint.
        if np.all(np.asarray(snap["hist_J"]) <= 0):
            self.ax_J.text(
                0.02, 0.92,
                "Numerical J-histogram is empty\n(check bin range or monotonicity)",
