"""
long_range_section6_gui_alltoall_only_v2_savepdf.py

Same as "alltoall_only" version, with ONE added feature:
- A "Save plots (PDF)" button that saves the current Matplotlib figure as a PDF.

Behavior:
- If you computed snapshots, the saved PDF corresponds to whatever shell is currently shown
  on the plots (i.e. the current slider position).
- You choose the filename via a standard file dialog.
...
class Section6LongRangeGUI:
    ...
    def _build_layout(self):
        ...
        # Buttons row 2: Save PDF (new)
        btns2 = ttk.Frame(left)
        btns2.pack(fill=tk.X, pady=(2, 2))
        btns2.columnconfigure(0, weight=1)
        self.btn_save_pdf = ttk.Button(btns2, text="Save plots (PDF)", command=self.on_save_pdf)
    ...
    def on_save_pdf(self):
        """
        Save the current figure (both subplots) to a PDF file.

        - The file dialog lets you pick the name and location.
        - We save at high quality (vector PDF).
        """
        default_name = "section6_longrange_plots.pdf"
    ...
    def _set_buttons_state(self, state: str):
        for b in [self.btn_compute, self.btn_play, self.btn_pause, self.btn_stop]:
            try:
                ...
                pass
        # Save button stays enabled unless you prefer otherwise.
